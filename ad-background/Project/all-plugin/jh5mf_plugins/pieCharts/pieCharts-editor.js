...
(function(JH5MF) {
    JH5MF.Editor.register("pieCharts",
        "", /*"l10n!/layouts/views/plugins/pieCharts-editor.tmpl",**/
        function(rootElement, jh5mf, baseLayout) {
            ...
            // The codes here is pieCharts only. 
            ...
            JH5MF.Editor.TrackEventEditor.extend(_this, jh5mf, _rootElement, {
                open: function(parentElement, trackEvent) {
                    ...
                    trackEvent.listen("dataChange.pieCharts", onAreaCountChange);
                    trackEvent.listen("trackeventupdated", onTrackEventUpdated);
                    // to setup property UI.
                    setup(trackEvent);
                },
                close: function() {
                    _trackEvent.unlisten("dataChange.pieCharts", onAreaCountChange);
                      ...
                      data: {
                        callback: function(data) {
                            ...
                            if (data.rows.length > 2) {
                                var obj = {};
                                $.each(outputStack, function (i, item) {
                                    obj[item]= parseInt(outputData[i]) + (parseInt(obj[item]) || 0);
                                });

                                for (var k in obj) {
                                    stackObj = {};
                                    stackObj["value"] = obj[k];
                                    stackObj["name"] = k;
                                    stackData.push(stackObj);
                                }

                                seriesData.push(stackData);
                            }
